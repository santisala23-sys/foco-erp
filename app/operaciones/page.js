  ...
  return (
    <div className="p-4 md:p-8 max-w-5xl mx-auto w-full pb-32">
      <header className="mb-8 border-b pb-4">
        ...
        <p className="text-slate-500 mt-1">Gestioná los entrenamientos y los resultados de los partidos.</p>
      </header>

      {/* TABS */}
      <div className="flex gap-4 mb-6 border-b border-slate-200 overflow-x-auto">
        <button onClick={() => handleTabChange('asistencia')}
          className={`pb-3 px-4 font-semibold text-sm transition-colors whitespace-nowrap relative ${vistaActiva === 'asistencia' ? 'text-blue-600' : 'text-slate-500 hover:text-slate-700'}`}>
          <div className="flex items-center gap-2"><Calendar size={18} /> Toma de Asistencia</div>
          {vistaActiva === 'asistencia' && <div className="absolute bottom-0 left-0 w-full h-0.5 bg-blue-600 rounded-t-full"></div>}
        </button>
        <button onClick={() => handleTabChange('partidos')}
          className={`pb-3 px-4 font-semibold text-sm transition-colors whitespace-nowrap relative ${vistaActiva === 'partidos' ? 'text-amber-600' : 'text-slate-500 hover:text-slate-700'}`}>
          <div className="flex items-center gap-2"><Trophy size={18} /> Registro de Partidos</div>
          {vistaActiva === 'partidos' && <div className="absolute bottom-0 left-0 w-full h-0.5 bg-amber-600 rounded-t-full"></div>}
        </button>
      </div>
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div>
            <label className="block text-sm font-medium text-slate-600 mb-1">Actividad</label>
            <select className="w-full border border-slate-300 rounded-lg p-2.5 bg-slate-50 outline-none"
          ...
          <div>
            ...
            <select className="w-full border border-slate-300 rounded-lg p-2.5 bg-slate-50 outline-none disabled:opacity-50"
          ...
          <div>
            <label className="block text-sm font-medium text-slate-600 mb-1">División</label>
            <select className="w-full border border-slate-300 rounded-lg p-2.5 bg-slate-50 outline-none disabled:opacity-50"
          ...
          <div>
            ...
            <input type="date" className="w-full border border-slate-300 rounded-lg p-2.5 bg-slate-50 outline-none"
        ...
        {vistaActiva === 'partidos' && (
          <div className="mt-6 pt-6 border-t border-slate-100 grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="md:col-span-2">
              <label className="block text-sm font-medium text-slate-600 mb-1 flex items-center gap-1"><Swords size={16}/> Rival</label>
              <input type="text" placeholder="Ej: Club Atlético..." className="w-full border border-slate-300 rounded-lg p-2.5 bg-slate-50 outline-none"
                value={datosPartido.rival} onChange={(e) => setDatosPartido({...datosPartido, rival: e.target.value})} />
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-600 mb-1">Condición</label>
              <select className="w-full border border-slate-300 rounded-lg p-2.5 bg-slate-50 outline-none"
            ...
            <div className="flex gap-2">
              <div className="w-1/2">
                <label className="block text-sm font-medium text-emerald-600 mb-1">GF</label>
                <input type="number" min="0" className="w-full border border-slate-300 rounded-lg p-2.5 bg-emerald-50 text-emerald-700 font-bold outline-none text-center"
                  value={datosPartido.goles_favor} onChange={(e) => setDatosPartido({...datosPartido, goles_favor: e.target.value})} />
              </div>
              <div className="w-1/2">
                <label className="block text-sm font-medium text-red-600 mb-1">GC</label>
                <input type="number" min="0" className="w-full border border-slate-300 rounded-lg p-2.5 bg-red-50 text-red-700 font-bold outline-none text-center"
        ...
        <div className="mt-6 flex justify-end">
          <button 
            ...
            className={`${vistaActiva === 'asistencia' ? 'bg-blue-600 hover:bg-blue-700' : 'bg-amber-600 hover:bg-amber-700'} text-white px-6 py-2.5 rounded-lg font-medium transition w-full md:w-auto disabled:opacity-50`}
          >
      ...
      {jugadores.length > 0 && (
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden relative">
          ...
          <div className="overflow-x-auto pb-20"> {/* pb-20 hace lugar para el botón flotante */}
            <table className="w-full text-left border-collapse min-w-[500px]">
              <thead>
                <tr className="bg-white border-b border-slate-200 text-sm text-slate-500">
                  ...
                  <th className="p-4 font-medium text-center">{vistaActiva === 'asistencia' ? 'Estado' : 'Goles Marcados'}</th>
                </tr>
              </thead>
              <tbody>
                {jugadores.map((jugador) => (
                  <tr key={jugador.id} className="border-b border-slate-100 hover:bg-slate-50 transition">
                    <td className="p-4 font-medium text-slate-800">{jugador.apellido}, {jugador.nombre}</td>
                    <td className="p-4 flex justify-center">
                            ...
                            <button key={estado} onClick={() => setAsistencias({ ...asistencias, [jugador.id]: estado })}
                              className={`px-3 py-1.5 text-sm rounded-md font-medium transition ${
                                asistencias[jugador.id] === estado 
                                  ? estado === 'Presente' ? 'bg-emerald-100 text-emerald-700 border border-emerald-300' : estado === 'Ausente' ? 'bg-red-100 text-red-700 border border-red-300' : 'bg-amber-100 text-amber-700 border border-amber-300'
                      ...
                      {vistaActiva === 'partidos' && (
                        <div className="flex items-center gap-3">
                          <button onClick={() => setRendimiento({...rendimiento, [jugador.id]: Math.max(0, rendimiento[jugador.id] - 1)})} className="w-8 h-8 rounded-full bg-slate-100 hover:bg-slate-200 flex items-center justify-center font-bold text-slate-600">-</button>
                          <span className="text-lg font-bold w-6 text-center">{rendimiento[jugador.id]}</span>
                          <button onClick={() => setRendimiento({...rendimiento, [jugador.id]: rendimiento[jugador.id] + 1})} className="w-8 h-8 rounded-full bg-amber-100 hover:bg-amber-200 flex items-center justify-center font-bold text-amber-700">+</button>
                        </div>
                      )}
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse min-w-[500px] mb-20"> {/* Margen extra abajo */}
              <thead>
                {/* ... encabezado igual ... */}
              </thead>
              <tbody>
          ...
          <div className="fixed bottom-6 right-6 md:right-10 z-[100]">
            <button 
              ...
              className={`
                ...
                ${vistaActiva === 'asistencia' ? 'bg-emerald-600' : 'bg-amber-600'}
              `}
            >
              <Save size={24} />
